...
public class Port2Method<HANDLER extends Method2Handler<R, P1, P2>, R, P1, P2> extends AbstractNonVoidMethod {
     ...
     */
    public Port2Method(PortInterface portInterface, String name, String p1Name, String p2Name, boolean handleInExtraThread, int defaultNetTimeout) {
     ...
     * @param defaultNetTimeout Default timeout for calls over the net (should be higher than any timeout for call to avoid that returning calls get lost)
     */
    public Port2Method(PortInterface portInterface, String name, String p1Name, String p2Name, boolean handleInExtraThread) {
    ...
    public void callAsync(InterfaceClientPort port, AsyncReturnHandler<R> handler, P1 p1, P2 p2, int netTimeout, boolean forceSameThread) {
        //1
        assert(hasLock(p1)); //2
        assert(hasLock(p2));
     ...
     */
    @SuppressWarnings("unchecked")
    public R call(InterfaceClientPort port, P1 p1, P2 p2, int netTimeout) throws MethodCallException {
        //1
        assert(hasLock(p1)); //2
        assert(hasLock(p2));
        InterfacePort ip = port.getServer();
