...
public class Port2Method<HANDLER extends Method2Handler<R, P1, P2>, R, P1, P2> extends AbstractNonVoidMethod {
    ...
    public void callAsync(@Const @Ptr InterfaceClientPort port, @Ptr AsyncReturnHandler<R> handler, @PassByValue @NoMatching P1 p1, @PassByValue @NoMatching P2 p2, @CppDefault("-1") int netTimeout, @CppDefault("false") boolean forceSameThread) {
        //1
        assert(hasLock(p1)); //2
        assert(hasLock(p2));
     ...
     */
    @SuppressWarnings("unchecked")
    public R call(InterfaceClientPort port, @PassByValue @NoMatching P1 p1, @PassByValue @NoMatching P2 p2, @CppDefault("-1") int netTimeout) throws MethodCallException {
        //1
        assert(hasLock(p1)); //2
        assert(hasLock(p2));
        InterfacePort ip = port.getServer();
